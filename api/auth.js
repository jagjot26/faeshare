...
router.post("/", async (req, res) => {
  ...
  try {
    const user = await UserModel.findOne({ email: email.toLowerCase() }).select("+password"); //In UserModel, in password, we set select to false. The password field will not be included when we search for a user.
    if (!user) {
